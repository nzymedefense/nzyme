...
public class Dot11BeaconFrameHandlerTest extends FrameHandlerTest {
    ...
    public void testDoHandle() throws MalformedFrameException, IllegalRawDataException {
        ...
        assertEquals(n.getAdditionalFields().get("_transmitter"), "00:c0:ca:95:68:3b");
        assertEquals(n.getAdditionalFields().get("_transmitter_fingerprint"), "dfac3abce0c722f9609343f7dfa208afa51a1c7decbd2eb6f96c78051f0a594b");
        assertEquals(n.getAdditionalFields().get("_ssid"), "WTF");
        assertEquals(n.getAdditionalFields().get("_security_full"), "WPA1-EAM-PSK-CCMP, WPA2-EAM-PSK-CCMP");
        assertEquals(n.getAdditionalFields().get("_is_wpa1"), true);
        assertEquals(n.getAdditionalFields().get("_is_wpa2"), true);
        assertEquals(n.getAdditionalFields().get("_is_wps"), false);
    ...
    public void testDoHandleBroadcast() throws MalformedFrameException, IllegalRawDataException {
        ...
        assertEquals(n.getAdditionalFields().get("_channel"), 1);
        assertEquals(n.getAdditionalFields().get("_transmitter"), "24:a4:3c:7d:01:cc");
        assertEquals(n.getAdditionalFields().get("_transmitter_fingerprint"), "52f519b9e8b1a4901a3db02407ff62246f5cfc2d5ddadd5a10e5230524ef04a9");
        assertEquals(n.getAdditionalFields().get("_ssid"), "[no SSID]");
        assertEquals(n.getAdditionalFields().get("_security_full"), "NONE");
        assertEquals(n.getAdditionalFields().get("_is_wpa1"), false);
        assertEquals(n.getAdditionalFields().get("_is_wpa2"), false);
        assertEquals(n.getAdditionalFields().get("_is_wps"), false);
        assertEquals(n.getAdditionalFields().get("_subtype"), "beacon");
    }
